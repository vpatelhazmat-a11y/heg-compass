  ...
  return useQuery({
    ...
    queryFn: async () => {
      const requests = {
        ...
        assessments: listRows("site_assessments", {
        ...
        opportunities: listRows("opportunities", {
...
function CommandCenter() {
  ...
  const overdueTasks = data.tasks.filter(
    (task) =>
      !["Completed", "Cancelled"].includes(task.status) && task.due_date && task.due_date < today,
  );
  const openTasks = data.tasks.filter((task) => !["Completed", "Cancelled"].includes(task.status));
  const liveBids = data.bids.filter(
    (bid) => !["Won", "Lost", "Withdrawn", "Cancelled", "Submitted"].includes(bid.status),
  );
  ...
  const openIncidents = data.incidents.filter((incident) =>
    ["Open", "In Process"].includes(incident.status),
  );
  ...
  const blocked = openOpportunities.filter(
    (opp) => opp.capacity_status && opp.capacity_status !== "Serviceable",
  );
  const capacityLost = data.lost.filter(
    (row) => row.capacity_issue || row.driver_issue || row.equipment_issue,
  );
  const capacityLostRevenue = capacityLost.reduce(
      ...
      <div className="space-y-6 p-6">
        ...
        <section aria-labelledby="today-heading" className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            />
            <StatTile
              label="Bids due in 14 days"
              value={bidsDue.length}
              tone={overdueBids.length ? "danger" : bidsDue.length ? "warning" : "neutral"}
              hint={overdueBids.length ? `${overdueBids.length} overdue` : undefined}
            />
            <StatTile
              label="Contract renewals (90 days)"
              value={renewals.length}
              tone={renewals.length ? "warning" : "neutral"}
            />
            <StatTile
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Panel title="Bids needing attention" description="Live bids sorted by due date">
            {bidsDue.length === 0 ? (
              <EmptyState
                title="No bid deadlines in the next two weeks"
                description="Add a bid to start tracking its deadlines."
              />
            ) : (
                ...
                {bidsDue.slice(0, 6).map((bid) => {
                  ...
                  return (
                    <li key={bid.id} className="flex items-center justify-between gap-3 py-2.5">
                      ...
                      <StatusBadge
                        status={due.label}
                        tone={due.tone === "neutral" ? "info" : due.tone}
                      />
                    </li>
                  );
                })}
          ...
          <Panel title="Open tasks" description="Yours and your team's">
            {openTasks.length === 0 ? (
              <EmptyState
                title="No open tasks"
                description="Tasks created from meetings, bids and incidents appear here."
              />
            ) : (
              <ul className="divide-y divide-border">
        ...
        <section aria-labelledby="capacity-heading" className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <StatTile
              ...
              value={formatMoney(capacityLostRevenue)}
              hint="Recorded by Sales"
            />
            <StatTile
          ...
          <section aria-labelledby="safety-heading" className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              ...
              <StatTile
                ...
                value={
                  data.assessments.filter((a) => (daysUntil(a.next_review_date) ?? 1) < 0).length
                }
                tone="danger"
              />
          ...
          <Panel title="Assessments due" className="lg:col-span-1">
            {assessmentsDue.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No site assessments are due in the next 90 days.
              </p>
            ) : (
              <ul className="space-y-2.5">
