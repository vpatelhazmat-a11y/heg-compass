...
export const Route = createFileRoute("/_authenticated/sites/$siteId")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Access, safety, routing and assessment knowledge for this site.",
      },
...
function SiteDetail() {
  ...
  const related = useQuery({
    ...
    queryFn: async () => {
      ...
      const [requirements, assessments, documents, incidents, lanes, equipment, productRates] =
        await Promise.all([
          ...
          listRows("incidents", { filters, order: { column: "incident_date", ascending: false } }),
          listRows("lanes", { anyOf: { origin_site_id: siteId, destination_site_id: siteId } }),
          listRows("current_equipment_assignments", { filters }),
          listRows("rates", {
            filters,
            select: "product_id, products!rates_product_id_fkey(id, product_name)",
          }),
        ]);
      ...
      return {
        ...
        products: [
          ...new Map(
            productRates.filter((row) => row.products).map((row) => [row.product_id, row.products]),
          ).values(),
        ],
      };
    },
