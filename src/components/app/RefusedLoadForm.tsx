...
import { isValidDate } from "@/lib/form-values";
import {
  useMemo,
  useState,
  useId,
...
import { insertRow, listRows, updateRow, type Row } from "@/lib/data";
...
export function RefusedLoadForm({
  ...
  const equipmentTypes = useLookup("refused_load_equipment_type", [
    "Product Tanker",
    "Box Van",
    "Waste Tanker",
    "Power Only",
  ...
  const lossReasons = useLookup("lost_reason");
  ...
  const validate = () => {
    ...
    else if (!isValidDate(form.call_in_date)) next["call_in_date"] = "That date is not valid.";
    ...
    if (!Number.isSafeInteger(loads) || loads < 1) next["load_count"] = "Enter at least one load.";
    ...
    if (form.offered_date && !isValidDate(form.offered_date))
      next["offered_date"] = "That date is not valid.";
    if (!form.cs_rep) next["cs_rep"] = "Choose the CS representative.";
    if (
      form.estimated_lost_revenue &&
      (!Number.isFinite(Number(form.estimated_lost_revenue)) ||
        Number(form.estimated_lost_revenue) < 0)
    )
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (!canEdit("refused_loads"))
        throw new Error("You do not have permission to edit refused loads.");
      const payload: TablesInsert<"refused_loads"> = {
