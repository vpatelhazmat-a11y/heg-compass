      ...
      >
        <div className="flex h-14 items-center gap-2 border-b border-sidebar-border px-4">
          ...
          <div className="leading-tight">
            <p className="text-sm font-semibold">Commercial Hub</p>
            <p className="text-[11px] text-sidebar-muted">HazMat Environmental Group</p>
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto px-2 py-3" aria-label="Main">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              className={cn(
                "mt-2 flex items-center gap-2.5 rounded-md px-3 py-2 text-sm transition-colors",
        ...
        <div className="border-t border-sidebar-border px-4 py-3 text-[11px] text-sidebar-muted">
          Trimble/TMW remains the system of record for operational data.
        </div>
      </aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border bg-surface px-4">
          ...
          <button
            ...
            className="flex h-9 w-full max-w-md items-center gap-2 rounded-md border border-input bg-background px-3 text-sm text-muted-foreground hover:border-border-strong"
          >
          ...
          <div className="ml-auto flex items-center gap-2">
            {canWrite && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-52">
                  ...
                  <DropdownMenuItem onSelect={() => setQuickCreate("customer")}>Customer</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setQuickCreate("opportunity")}>Opportunity</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setQuickCreate("bid")}>Bid</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setQuickCreate("equipment")}>Equipment</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setQuickCreate("task")}>Task</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setQuickCreate("document")}>Document</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex h-9 items-center gap-2 rounded-md px-2 text-sm hover:bg-accent">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary text-xs font-semibold text-primary-foreground">
                    {initials}
                  </span>
                  <span className="hidden max-w-[10rem] truncate sm:inline">{session?.fullName ?? "Account"}</span>
                </button>
              </DropdownMenuTrigger>
